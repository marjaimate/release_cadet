module ReleaseCadet
  class Features < Command
    ...
    def execute release, against=nil
      ...
      output << `git log --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr %an)%Creset' --abbrev-commit --date=relative origin/#{against}..origin/#{branch} --merges`
    ...
    def get_merged_branches_in branch
      `git branch -r --merged origin/#{branch}`.split("\n").map do |a| 
